...
static void all_md5_tests(void) {
  ...
  test_md5_cpu();
  //
  // intel/amd: md5_cpu_avx() tests --- comparison with the hash data computed
  // by test_cpu_md5()
  //
#ifdef MD5_CPU_AVX
  test_md5_cpu_avx();
#endif
  //
  // intel/amd: md5_cpu_avx2() tests --- comparison with the hash data computed
  // by test_cpu_md5()
  //
#ifdef MD5_CPU_AVX2
  test_md5_cpu_avx2();
#endif
  //
  // arm: md5_cpu_neon() tests --- comparison with the hash data computed by
  // test_cpu_md5()
  //
#ifdef MD5_CPU_NEON
  test_md5_cpu_neon();
#endif
  //
  // cuda: md5_cuda() tests --- comparison with the hash data computed by
  // test_cpu_md5()
  //
#ifdef MD5_CUDA
  test_md5_cuda();
...
static void alarm_signal_handler(int dummy) { stop_request = 1; }
...
int main(int argc, char **argv) {
  ...
  if (argc == 2 && argv[1][0] == '-' && argv[1][1] == 't') {
    ...
    test_next_value_to_try_ascii(); // this will help warming up (turbo boost)
                                    // the processor!
#endif
    all_md5_tests();
    return 0;
  ...
  if ((argc >= 2 && argc <= 4) && argv[1][0] == '-' && argv[1][1] == 's') {
    ...
    seconds = (argc > 2) ? parse_time_duration(argv[2]) : 0u;
    if (seconds == 0u) {
      fprintf(
          stderr,
          "main: bad number of seconds --- format [Nd][Nh][Nm][N[s]], where "
          "each N is a number and [] means whats inside it is optional\n");
      exit(1);
    }
    if (seconds < 15u)
      seconds = 15u; // at least 15 seconds
    if (seconds > 7200u)
  ...
  fprintf(stderr,
          "usage: %s -t                               # MD5 hash tests\n",
          argv[0]);
  fprintf(stderr,
          "       %s -s0 [seconds] [ignored]          # search for DETI coins "
          "using md5_cpu()\n",
          argv[0]);
#ifdef DETI_COINS_CPU_AVX_SEARCH
  fprintf(stderr,
          "       %s -s1 [seconds] [n_random_words]   # search for DETI coins "
          "using md5_cpu_avx()\n",
          argv[0]);
#endif
#ifdef DETI_COINS_CPU_AVX2_SEARCH
  fprintf(stderr,
          "       %s -s2 [seconds] [n_random_words]   # search for DETI coins "
          "using md5_cpu_avx2()\n",
          argv[0]);
#endif
#ifdef DETI_COINS_CPU_NEON_SEARCH
  fprintf(stderr,
          "       %s -s3 [seconds] [n_random_words]   # search for DETI coins "
          "using md5_cpu_neon()\n",
          argv[0]);
#endif
#ifdef DETI_COINS_CUDA_SEARCH
  fprintf(stderr,
          "       %s -s4 [seconds] [n_random_words]   # search for DETI coins "
          "using CUDA\n",
          argv[0]);
#endif
#ifdef DETI_COINS_CPU_SPECIAL_SEARCH
  fprintf(stderr,
          "       %s -s9 [seconds] [ignored]          # special search for "
          "DETI coins using md5_cpu()\n",
          argv[0]);
#endif
  fprintf(stderr, "                                           #   seconds is "
                  "the amount of time spent in the search\n");
  fprintf(stderr, "                                           #   "
                  "n_random_words is the number of 4-byte words to use\n");
  return 1;
}
