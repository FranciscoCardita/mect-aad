...
static void deti_coins_cpu_avx2_search() {
  u32_t interleaved_coins[13 * N_LANES] __attribute__((aligned(32)));
  u32_t interleaved_hashes[4 * N_LANES] __attribute__((aligned(32)));
  u08_t *bytes[N_LANES];
  ...
  for (n_attempts = n_coins = 0; stop_request == 0; n_attempts += N_LANES) {
    md5_cpu_avx2((v8hi *)interleaved_coins, (v8hi *)interleaved_hashes);
